...
// only within CODE_RECOVERY_GRACE_MS of the code's creation. Past that, a row
// still sitting at 1 cannot be a live race (a real resolution finishes in
...
export async function consumeCode(code, opts = {}, now = Date.now()) {
  return attemptConsume(code, opts, now, /* allowRecovery */ true);
}

// A crash between the guarded UPDATE below (claiming IN_FLIGHT) and this
// function's own resolution of that row (further down) leaves the row
// wedged at IN_FLIGHT forever - nothing else ever moves it out. Read at face
// value, "still IN_FLIGHT" means "someone else is presenting this code right
// now", so the legitimate retry that follows such a crash was being treated
// as a leak: `invalid_grant`, and the user's SSO session deleted out from
// under them. `allowRecovery` bounds the one-time reclaim-and-retry below to
// a single attempt, so recovering a wedged row can never loop.
async function attemptConsume(code, { clientId, redirectUri } = {}, now, allowRecovery) {
  ...
  if (changes !== 1) {
    ...
    if (spent.used === OUTCOME.SUCCESS) {
      ...
      const wedged = allowRecovery && spent.created_at <= now - CODE_RECOVERY_GRACE_MS;
      if (wedged) {
        // Past the grace period this cannot still be a genuine concurrent
        // presentation - a real resolution finishes in milliseconds - so
        // this is a wedge, not a leak. Reclaim it as unused (guarded, so a
        // second caller racing this same recovery only ever wins once) and
        // resolve it exactly as if this were the first presentation.
        await dbRun('UPDATE codes SET used = ? WHERE code_hash = ? AND used = ?', [
          OUTCOME.UNUSED,
          hash,
          OUTCOME.IN_FLIGHT
        ]);
        return attemptConsume(code, { clientId, redirectUri }, now, false);
      }
      // Still within the grace period: two simultaneous exchanges of one
      // code is a leak of the code itself (spec §7.5).
      if (spent.sso_session_id) await deleteSessionById(spent.sso_session_id);
      return { ok: false, reason: 'replayed' };
    }
