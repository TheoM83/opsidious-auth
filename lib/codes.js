...
export async function consumeCode(code, { clientId, redirectUri } = {}, now = Date.now()) {
  ...
  // Validation passed: mark as successfully consumed, and in the same UPDATE
  // null every field that could otherwise go on linking an account to a named
  // application for the rest of this row's tombstone lifetime (up to
  // CODE_TTL_MS + SWEEP_INTERVAL_MS). `row`, above, was already read before
  // this UPDATE runs, so the caller (routes/token.js) still gets live values.
  // Replay detection below only ever branches on `used` and `sso_session_id`,
  // both of which survive (spec §4.2, §7.5).
  await dbRun(
