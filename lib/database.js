...
const SCHEMA = [
  ...
  `CREATE TABLE IF NOT EXISTS auth_requests (
     ...
     -- La langue dans laquelle la connexion a été demandée. Un aller-retour par
     -- Google ne doit pas changer la langue de la page où l'on revient : le
     -- navigateur peut très bien annoncer autre chose que ce que l'application
     -- avait demandé, et c'est l'application qui a demandé en premier.
     --
     -- Ce n'est pas une donnée sur la personne : elle vaut au plus 'fr' ou 'en',
     -- elle est écrite par le service et non par le client, et la ligne entière
     -- est supprimée au retour de Google.
     ui_locale             TEXT,
  ...
  //
  // `app_sub`, `client_id`, `redirect_uri`, `nonce`, `code_challenge` and
  // `code_challenge_method` are nullable, not just NOT-NULL-at-insert: once a
  // code is successfully consumed, `consumeCode` nulls all six in the same
  // UPDATE that marks it spent, so this row cannot go on linking an account
  // (via `sso_session_id` -> `sso_sessions` -> `accounts`) to a named
  // application for the rest of its tombstone lifetime. Only
  // `sso_session_id`, `code_hash`, `used`, `expires_at` and `created_at` need
  // to survive - replay detection, session revocation and the stale-wedge
  // check in `consumeCode` use exactly those (spec §4.2, §7.5).
  ...
  // Combien ce service a été sollicité, et rien sur qui l'a sollicité : une
  // ligne par heure, un compteur dedans. Pas d'adresse, pas de chemin, pas de
  // session — rien de joignable à quoi que ce soit. Deux personnes et une
  // personne qui recharge deux fois donnent la même ligne, délibérément.
...
export async function migrateSchema() {
  ...
  // Le nom de la table est interpolé, pas lié : `ALTER TABLE ?` est une erreur
  // de syntaxe SQLite, tout comme `PRAGMA busy_timeout = ?` plus haut. La
  // valeur vient d'un tableau littéral de ce fichier, jamais d'une entrée de
  // requête, exactement la même justification que celle déjà consignée pour
  // le PRAGMA busy_timeout.
  if (!(await hasColumn('auth_requests', 'ui_locale'))) {
    await dbRun('ALTER TABLE auth_requests ADD COLUMN ui_locale TEXT');
    console.info('migration : colonne auth_requests.ui_locale ajoutée');
  }
...
async function sealExistingSecrets() {
  ...
  const pepper = await dbGet('SELECT value, kdf_salt FROM settings WHERE key = ?', ['lookup_pepper']);
