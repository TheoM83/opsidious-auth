...
import { KEY_ROTATION_MS, KEY_GRACE_MS, MASTER_KEY } from './config.js';
import { INFO_KEYSTORE, sealUnderMaster, openUnderMaster } from './crypto.js';
...
async function mint(now) {
  ...
  // Scellée avant d'être écrite. En clair, ce champ suffisait à forger un jeton
  // portant n'importe quel sujet — vérifié en le faisant, avec pour seule
  // entrée le fichier de base.
  const { kdfSalt, blob } = sealUnderMaster(MASTER_KEY, INFO_KEYSTORE, await exportPKCS8(privateKey));

  await dbRun(
    `INSERT INTO signing_keys (kid, private_pem, pem_kdf_salt, public_jwk, created_at, retires_at, expires_at)
     VALUES (?,?,?,?,?,?,?)`,
    [kid, blob, kdfSalt, JSON.stringify(jwk), now, retiresAt, retiresAt + KEY_GRACE_MS]
  );
  return kid;
}
...
async function toSigner(row) {
  const pem = openUnderMaster(MASTER_KEY, INFO_KEYSTORE, row.pem_kdf_salt, row.private_pem);
  return { kid: row.kid, privateKey: await importPKCS8(pem.toString('utf8'), 'RS256') };
}
