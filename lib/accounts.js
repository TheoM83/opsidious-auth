...
export async function ensurePepper() {
  ...
  const existing = await dbGet('SELECT value, kdf_salt FROM settings WHERE key = ?', [
    'lookup_pepper'
  ]);
  if (existing) {
    return openUnderMaster(MASTER_KEY, INFO_PEPPER, existing.kdf_salt, existing.value).toString(
      'utf8'
    );
  }
