...
// An explicit ceiling on our own call to Google. Without it, undici leaves a
// hung connection open for roughly five minutes on a stalled response - and
// by the time that connection would time out on its own, /callback/google
// has long since deleted the parked auth_requests row, so nothing can ever
// use the answer. That just holds a connection and a rate-limit slot open
// for a request that cannot possibly succeed. The client package this
// service ships (client/index.js's timedFetch) already gets this right;
// this is the same defence on the service's own outbound call.
...
export async function exchangeCode(code) {
  ...
  const timer = setTimeout(() => controller.abort(), GOOGLE_TOKEN_TIMEOUT_MS);
  let res;
  try {
    res = await transport.fetchImpl(TOKEN, {
      method: 'POST',
